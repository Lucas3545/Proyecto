    ...
    if (AI_CONFIG.CHATBOT.enabled) {
        try {
            window.chatbot = new AIChatbot(AI_CONFIG.OPENAI_API_KEY, {
                ...
                chatEndpoint: window.CHATBOT_API_ENDPOINT || './back/PHP/ai_chatbot_proxy.php'
            });
...
function showAISetupHelp() {
    console.log('Para activar IA en servidor define OPENAI_API_KEY y usa el proxy back/PHP/ai_chatbot_proxy.php');
}
